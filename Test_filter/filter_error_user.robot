...
Check error messages when filtering (Z to A)
    [Documentation]    Kiểm tra thông báo lỗi khi lọc (Z to A)
    Login To Website     ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    Page Should Contain    Products
    Select From List By Label    css=.product_sort_container    Name (Z to A)
    Get Error Message
...
Check error messages when filtering (low to high)
    [Documentation]   Kiểm tra thông báo lỗi khi lọc  (low to high)
    Login To Website     ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    Page Should Contain    Products
    Select From List By Label    css=.product_sort_container    Price (low to high)
    Get Error Message
...
Check error messages when filtering (high to low)
    [Documentation]    Kiểm tra thông báo lỗi khi lọc  (high to low)
    Login To Website     ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    Page Should Contain    Products
    Select From List By Label    css=.product_sort_container    Price (high to low)
    Get Error Message
...
Check error message when filtering while waiting for cookies to run out (A to Z)
    [Documentation]    Kiểm tra chức năng lọc khi chờ hết cookie (A to Z)
    Login To Website     ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    Page Should Contain    Products
    Select From List By Label    css=.product_sort_container    Name (A to Z)
    Get Error Message
...
Check error message when filtering while waiting for cookies to run out (Z to A)
    [Documentation]    Kiểm tra chức năng lọc khi chờ hết cookie (Z to A)
    Login To Website     ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    Page Should Contain    Products
    Select From List By Label    css=.product_sort_container    Name (Z to A)
    Get Error Message
...
Check error message when filtering while waiting for cookies to run out (low to high)
    [Documentation]   Kiểm tra thông báo lỗi khi lọc khi chờ hết cookie  (low to high)
    Login To Website     ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    Page Should Contain    Products
    Select From List By Label    css=.product_sort_container    Price (low to high)
    Get Error Message
...
Check error message when filtering while waiting for cookies to run out (high to low)
    [Documentation]    Kiểm tra thông báo lỗi khi lọc khi chờ hết cookie (high to low)
    Login To Website     ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    Page Should Contain    Products
    Select From List By Label    css=.product_sort_container    Price (high to low)
    Get Error Message
...
Get Error Message
    ${error_message}=    Run Keyword And Return Status    Page Should Contain    Error    # Kiểm tra sự xuất hiện của thông báo lỗi
    Run Keyword If    ${error_message}    Log Error Message and Stop Test    # Nếu có thông báo lỗi, log và dừng test case
    ...    ELSE    Log    ${FALSE}    # Nếu không có thông báo lỗi, log FALSE

Log Error Message and Stop Test
    Alert Should Be Present   Sorting is broken! This error has been reported to Backtrace.
    Alert Should Not Be Present   
    Log    Test case is passed.    # Log thông báo test case đã passed
    Fail    Test case passed due to absence of error message.    # Dừng test case và đánh dấu là passed
