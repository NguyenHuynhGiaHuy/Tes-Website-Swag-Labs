...
Compare Button "Check out" Position
    ...
    ${button_position_erro} =    Execute JavaScript    return document.querySelector('.btn_action').getBoundingClientRect();
    Close Browser

    Compare Positions     ${button_position_standard}    ${button_position_erro}
     [Teardown]    Close All Browsers
...
Compare Button "Add to cart" Position
    ...
    Login To Website   ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    ${button_position_erro} =    Execute JavaScript    return document.querySelector('.btn_primary').getBoundingClientRect();
    Close Browser
