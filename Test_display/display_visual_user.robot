...
Compare Button "Check out" Position
    ...
    ${button_position_visual} =    Execute JavaScript    return document.querySelector('.btn_action').getBoundingClientRect();
    Close Browser

    Compare Positions     ${button_position_standard}    ${button_position_visual}
   [Teardown]    Close All Browsers
...
Compare Button "Add to cart" Position
    ...
    Login To Website   ${URL}    ${BROWSER}    ${VALID_USERNAME}    ${VALID_PASSWORD}
    ${button_position_visual} =    Execute JavaScript    return document.querySelector('.btn_primary').getBoundingClientRect();
    Close Browser
...
Compare footer content positions
    [Documentation]    So sánh vị trí nội dung của footer giữa các người dùng
    Login To Website   ${URL}    ${BROWSER}    ${STANDARD_USER_USERNAME}    ${VALID_PASSWORD}

Compare the position of the "X" icon in the menu
    [Documentation]    So sánh vị trí của icon "X" trong memu giữa các người dùng giữa các người dùng
    Login To Website   ${URL}    ${BROWSER}    ${STANDARD_USER_USERNAME}    ${VALID_PASSWORD}

Compare the position of the "open menu" icon
    [Documentation]    So sánh vị trí của icon "open menu" giữa các người dùng
    Login To Website   ${URL}    ${BROWSER}    ${STANDARD_USER_USERNAME}    ${VALID_PASSWORD}
