...
TC08 Logout After Moving Product Detail Page, Add Product To Cart
...
TC09 Logout After Adding Products Not Found To The Cart And Checking The Cart
    [Documentation]    Kiểm tra xem đăng xuất sau khi thêm sản phẩm không tìm thấy vào giỏ hàng và kiểmt tra giỏ hàng
    Input Username And Password    ${USERNAME}    ${PASSWORD}
    See Product Detail And Add To Cart    ${ITEM_NOT_FOUND}    #Xem sản phẩm bị lỗi không tìm thấy và thêm vào giỏ hàng
    Click Link    //a[@class='shopping_cart_link']
    Verify Is_Visible Element    //a[text()='Logout']    #Kiểm tra xem có nút logout trên màn hình không

TC10 Log Out After Going To The Checkout Page And Entering Your Payment Information
...
TC11 Log Out After Going To The Checkout Page And Don't Entering Your Payment Information
...
TC12 Log Out After Going To The Checkout Page And Entering Invalid Payment Information
...
TC13 Log Out After Going To The Checkout Page And Entering Payment Information With Blanks
...
TC14 Log Out After Going To The Checkout Page And Entering Payment Information Is Too Short
...
TC15 Log Out After Going To The Checkout Page And Entering Payment Information Is Too Long
...
TC16 Log Out After Entering Payment Information But Not Confirming The Finish
...
TC17 Log out after successful payment
    [Documentation]    đăng xuất sau khi thanh toán thành công
    Input Username And Password    ${USERNAME}    ${PASSWORD}
    Check Cart And Checkout
    Fill Payment Info    firstname    lastname    12345
    Click Element    continue
...
Add Product To Cart
    ...
    Click Button    ${Add_to_cart}    #Thêm sản phẩm vào giỏ hàng
...
See Product Detail And Add To Cart
    [Arguments]    ${Item_detail}
    Click Element    ${Item_detail}    #Chọn xem chi tiết sản phầm bị lỗi
